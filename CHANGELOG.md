...
### Added
- `AnalysisLoader(measurement_id, analysis_id=..., analysis_type=..., name=...)`
  reads one of a measurement's stored analyses (for example degradation modes
  per RPT) as a fit's data, the feature-table counterpart to
  `DataLoader.from_db`. At least one selector is required, and a selector
  matching more than one analysis raises rather than picking one.
- `window_rows(frame, time_range)` keeps the rows of a table whose `Time [s]`
  falls inside a `{"start", "end"}` window in seconds.

### Fixed
- A `time_range` given alongside `first_step` / `last_step` is applied after the
  steps are chosen, by narrowing the selected slice. Previously the window was
  applied first, so step positions shifted: a fit could fail with an index
  error, or silently run on data offset from the steps it named.
- Maccor data records which procedure call ran each step in a new `Frame`
  column, read from the file's call codes, so steps in sibling sub-procedures
  are no longer merged or nested under each other. Step numbering starts a new
  step wherever the frame changes.

### Changed
- Requires `ionworks-api>=0.34.0`, which provides the `client.analysis.find_one`
  lookup that `AnalysisLoader` reads through.

## [0.17.0] - 2026-09-22

### Added
- `DataLoader.generate_experiment` takes `termination` and `period`. Under the
  default `termination="duration"` each step replays its measured length, as
  before; `termination="events"` instead replays the endpoint each step reached
  in the variable it was controlling, so a capacity disagreement reads as
  capacity to that endpoint rather than as a residual voltage. `period` sets the
  sample interval, defaulting to each step's own mean interval.

### Changed
- `generate_experiment` keeps the cycle structure of the loaded data rather than
  flattening every step into one cycle, so a generated experiment cycles the way
  the measurement did.
- `use_cv=True` emits constant-voltage steps as real voltage holds, simulated
  rather than driven with the measured current. Pair it with
  `termination="events"`: under `"duration"` the preceding step stops on the
  clock, and a hold entered below its setpoint draws whatever current closes the
  gap.

### Fixed
- A step whose indices no longer select its own samples is now rejected with a
  clear error instead of silently generating a step built from another step's
  data.

## [0.16.0] - 2026-09-07

### Added
- Header metadata is extracted for every cycler format, not just Maccor. Novonix,
  Basytec, Biologic, Arbin, Neware, Gamry, Digatron and Repower each read their
  own header block, where previously they fell back to reporting only the reader
  name and start time.
- Readers translate their native labels into one shared vocabulary, so
  `test_setup` keeps the same shape across cyclers. Maccor's `Procedure`,
  Novonix's `Protocol`, Basytec's `Testplan`, Arbin's `Schedule File Name` and
  Neware's `Step file` all arrive as `protocol["name"]`; Arbin's `Creator` and
  Biologic's `User` both arrive as `operator`.
- A header field that maps to no known key is dropped rather than passed through
  under its native name, so the metadata surface stays the same across formats
  and widening it is a deliberate change in one place.
