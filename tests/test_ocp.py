...
class TestOcpSteps:
    """Tests for ionworksdata.steps.ocp_steps."""

    def test_basic_output_shape(self):
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0, 3.8, 3.6],
                "Capacity [A.h]": [0.0, 0.1, 0.2, 0.3],
                "Step count": [0, 0, 0, 0],
                "Cycle count": [0, 0, 0, 0],
            }
        )
        steps = iwdata.steps.ocp_steps(ts)
        assert isinstance(steps, pl.DataFrame)
        assert steps.shape[0] == 1

    def test_voltage_stats(self):
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0, 3.8, 3.6],
                "Capacity [A.h]": [0.0, 0.1, 0.2, 0.3],
        ...
        steps = iwdata.steps.ocp_steps(ts)
        row = steps.row(0, named=True)
        assert row["Start voltage [V]"] == pytest.approx(4.2)
        assert row["End voltage [V]"] == pytest.approx(3.6)
        assert row["Min voltage [V]"] == pytest.approx(3.6)
        assert row["Max voltage [V]"] == pytest.approx(4.2)

    def test_discharge_capacity(self):
        """Capacity increases (voltage decreasing) → discharge."""
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0, 3.8],
        ...
        steps = iwdata.steps.ocp_steps(ts)
        row = steps.row(0, named=True)
        assert row["Discharge capacity [A.h]"] == pytest.approx(1.0)
        assert row["Charge capacity [A.h]"] == pytest.approx(0.0)

    def test_charge_capacity(self):
        """Capacity decreases → charge."""
        ts = pl.DataFrame(
            {
                "Voltage [V]": [3.6, 3.8, 4.0],
                "Capacity [A.h]": [1.0, 0.5, 0.0],
                "Step count": [0, 0, 0],
            }
        )
        steps = iwdata.steps.ocp_steps(ts)
        row = steps.row(0, named=True)
        assert row["Discharge capacity [A.h]"] == pytest.approx(0.0)
        assert row["Charge capacity [A.h]"] == pytest.approx(1.0)

    def test_no_capacity_column(self):
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0, 3.8],
                "Step count": [0, 0, 0],
        ...
        steps = iwdata.steps.ocp_steps(ts)
        row = steps.row(0, named=True)
        assert row["Discharge capacity [A.h]"] == 0.0
        assert row["Charge capacity [A.h]"] == 0.0

    def test_step_type_is_ocp(self):
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0, 3.8],
                "Step count": [0, 0, 0],
        ...
        steps = iwdata.steps.ocp_steps(ts)
        assert steps["Step type"][0] == "OCP"

    def test_current_power_fields_are_zero(self):
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0, 3.8],
                "Step count": [0, 0, 0],
        ...
        steps = iwdata.steps.ocp_steps(ts)
        row = steps.row(0, named=True)
        for prefix in ("current [A]", "power [W]", "frequency [Hz]"):
            for stat in ("Min ", "Max ", "Std ", "Mean "):
                assert row[stat + prefix] == 0.0

    def test_indices(self):
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0, 3.8, 3.6, 3.4],
    ...
    def test_cycle_capacity_columns_present(self):
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0, 3.8],
                "Capacity [A.h]": [0.0, 0.5, 1.0],
        ...
        assert "Cycle discharge energy [W.h]" in steps.columns
        assert "Cycle charge energy [W.h]" in steps.columns

    def test_integer_fields_not_null(self):
        """Step from cycler and Cycle from cycler must be integers (not null)
        because the cloud's MeasurementStepBase model requires it."""
        ts = pl.DataFrame(
            {
                "Voltage [V]": [4.2, 4.0],
                "Step count": [0, 0],
        ...
        steps = iwdata.steps.ocp_steps(ts)
        row = steps.row(0, named=True)
        assert row["Step from cycler"] is not None
        assert row["Cycle from cycler"] is not None
