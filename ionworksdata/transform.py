...
from ionworks.validators import (
    CAPACITY_ENERGY_INTEGRAL_TOLERANCE,
    positive_current_is_charge,
    running_step_reset_integral,
    step_boundary_idx,
    validate_positive_current_is_discharge,
    worst_row_relative_error,
)
import iwutil
import numpy as np
import pandas as pd
...
def get_cumulative_step_number(
    ...
    if combined_options["group EIS steps"] and ("Frequency [Hz]" in data.columns):
        ...
        # Filled because comparing through a null gives an object array.
        is_eis = (freq.fill_null(0.0) > combined_options["EIS tolerance"]).to_numpy()
        eis_boundary = np.concatenate([[False], is_eis[1:] != is_eis[:-1]])
        step_changes[is_eis] = 0
        step_changes[eis_boundary] = 1

    # A callee can open on the number its caller's call step had.
    if "Frame" in data.columns:
        frame = data.get_column("Frame").to_numpy()
        step_changes[1:][frame[1:] != frame[:-1]] = 1
...
def _calculate_capacity(
    ...
    # Otherwise, calculate using cumulative trapezoidal integration with
    # a per-step reset that matches the platform's reported columns.
    t = data.get_column("Time [s]").to_numpy()
    boundary = step_boundary_idx(step_numbers) if step_numbers is not None else None
    discharge_capacity = running_step_reset_integral(
        np.maximum(current, 0), t, boundary_idx=boundary
    )
    charge_capacity = running_step_reset_integral(
        np.maximum(-current, 0), t, boundary_idx=boundary
    )
    return discharge_capacity, charge_capacity


# Time-domain quantities an EIS sweep does not measure.
_EIS_ABSENT_COLUMNS = (
...
def clear_time_domain_columns_on_eis_rows(data: pl.DataFrame) -> pl.DataFrame:
    """Null the time-domain quantities on frequency-domain rows.

    A sweep interleaved into cycling data carries no time, so a cumulative
    quantity derived across it comes back NaN. The quantity is absent on those
    rows, not not-a-number, and NaN would propagate into any later arithmetic.

    Targets rows that are *untimed* rather than every row with a frequency:
    Gamry and BioLogic report EIS with a real ``Time [s]``, and their
    cumulative quantities are meaningful.

    Parameters
    ----------
    data : pl.DataFrame
        Time series that may contain interleaved EIS rows.

    Returns
    -------
    pl.DataFrame
        The data with capacity, energy and power nulled on untimed EIS rows.
        Returned unchanged unless both ``Frequency [Hz]`` and ``Time [s]``
        are present.
    """
    if "Frequency [Hz]" not in data.columns or "Time [s]" not in data.columns:
        return data

    is_untimed_sweep = (
        pl.col("Time [s]").is_null() & pl.col("Frequency [Hz]").is_not_null()
    )
    return data.with_columns(
        [
            pl.when(is_untimed_sweep).then(None).otherwise(pl.col(col)).alias(col)
            for col in _EIS_ABSENT_COLUMNS
...
def set_power(data: pl.DataFrame) -> pl.DataFrame:
    """
    Calculate power for the data and assign it to a new column called "Power [W]".

    Power is computed as "Voltage [V]" * "Current [A]", so it inherits the sign
    convention of current (positive on discharge, negative on charge). If a
    "Power [W]" column is already present (e.g. uploaded from the cycler), it is
    left untouched rather than overwritten.

    Parameters
    ----------
    data : pl.DataFrame
        The data to calculate the power for. Must contain "Voltage [V]" and
        "Current [A]" columns unless "Power [W]" is already present.

    Returns
    -------
    pl.DataFrame
        The data with a "Power [W]" column added, unless one already existed.
    """
    if "Power [W]" in data.columns:
        return data
    if "Voltage [V]" not in data.columns or "Current [A]" not in data.columns:
        return data
    return data.with_columns(
...
def resign_power_from_current(data: pl.DataFrame) -> pl.DataFrame:
    """
    Re-sign an existing "Power [W]" column to match "Current [A]"'s sign,
    preserving the column's magnitude.

    Used when a native (cycler-supplied) "Power [W]" column was computed
    under a current sign convention that has since been corrected (e.g. by
    ``set_positive_current_for_discharge`` or a validator-triggered sign
    flip). The magnitude of a native power reading is authoritative even if
    it doesn't exactly equal Voltage [V] * Current [A] (different sampling
    instant, calibration, etc.), so only its sign is re-derived — the same
    principle ``_split_cumulative_by_direction`` applies to a native
    capacity/energy column's direction. A no-op if "Power [W]" or
    "Current [A]" is absent.

    Where "Current [A]" is exactly zero, null, or NaN, ``sign()`` gives no
    usable direction; the row's own existing "Power [W]" value is kept
    as-is rather than being zeroed, nulled, or NaN'd out, so a nonzero
    relaxation-period power reading is never silently destroyed.

    Parameters
    ----------
    data : pl.DataFrame
        The data containing the "Power [W]" column to re-sign.

    Returns
    -------
    pl.DataFrame
        The data with "Power [W]" re-signed to match "Current [A]", or
        unchanged if either column is absent.
    """
    if "Power [W]" not in data.columns or "Current [A]" not in data.columns:
        return data
    current_sign = pl.col("Current [A]").sign()
    # (current_sign == 0) is null (falsy in `when`) when Current [A] is null,
    # and sign(NaN) is NaN (also not caught by == 0), so null and NaN current
    # must both be checked explicitly alongside zero current — otherwise
    # abs(Power) * null/NaN would silently null/NaN out the row below.
    no_usable_sign = (
        current_sign.is_null() | current_sign.is_nan() | (current_sign == 0)
    )
    return data.with_columns(
        pl.when(no_usable_sign)
        .then(pl.col("Power [W]"))
        .otherwise(pl.col("Power [W]").abs() * current_sign)
        .alias("Power [W]")
    )
...
def _calculate_energy(
    ...
    t = data.get_column("Time [s]").to_numpy()
    boundary = step_boundary_idx(step_numbers) if step_numbers is not None else None
    discharge_energy = running_step_reset_integral(
        np.maximum(power, 0), t, boundary_idx=boundary
    )
    charge_energy = running_step_reset_integral(
        np.maximum(-power, 0), t, boundary_idx=boundary
    )
    return discharge_energy, charge_energy
...
def _scaled_rest_tol(current_np: np.ndarray) -> float:
    """Return the rest-step threshold for a given current array.

    The absolute floor in ``iwdata.settings.get_rest_tol()`` (default 1 mA) is
    too coarse for coin/half-cell data where the cycling current itself is
    below 1 mA. Scale down by ``zero_current_percent_tol`` times the largest
    observed |I|, and take the ``min`` of the two so the threshold never
    *exceeds* the absolute floor on noisy normal-current files.
    """
    rest_tol_abs = iwdata.settings.get_rest_tol()
    if len(current_np) == 0:
        return rest_tol_abs
    abs_max_i = float(np.abs(current_np).max())
    if abs_max_i == 0:
        return rest_tol_abs
    return min(rest_tol_abs, abs_max_i * iwdata.settings.get_zero_current_percent_tol())
...
def set_positive_current_for_discharge(
    ...
    rest_tol = _scaled_rest_tol(current_np)
...
#: Vendor direction spellings, mapped to "C"/"D". Matched whole-string, never by
#: prefix: Neware writes both "CC Chg" and "CC DChg".
MODE_LABELS = {
    "C": "C",
    "CHA": "C",
    "CHG": "C",
...
def _fix_sign_from_mode_column(
    ...
    """Use a mode column (``"Status"``) to set the current's sign.
    ...
    ).replace_strict(MODE_LABELS, default=None)

    labelled = set(data.select(mode.alias("m")).get_column("m").unique().to_list()) & {
        "C",
        "D",
    }
    if not labelled:
    ...
    # With both labels present, an unknown one is a rest/marker state. With only
    # one, it could be the other direction, and would keep an inverted sign.
    if labelled != {"C", "D"}:
        resolved = data.select(mode.alias("m"), pl.col(current_col).abs().alias("i"))
        rest_tol = _scaled_rest_tol(resolved.get_column("i").to_numpy())
        unresolved = resolved.filter(
            (pl.col("i") > rest_tol) & pl.col("m").is_null()
        ).height
        if unresolved:
            return None

    # Set the sign, don't negate charge rows: that assumed absolute values and
    # left an already-negative discharge negative.
...
def _prepare_step_data(
    ...
    *summary_no_rest* contains only non-rest steps.
    ...
    # Only derive ``Step count`` if the caller hasn't already provided one
    # (e.g. from the cycler's authoritative step id). Re-deriving from
    # current-sign changes can produce different groupings on noisy data.
    if "Step count" not in work.columns:
    ...
    rest_tol = _scaled_rest_tol(work.get_column(current_col).to_numpy())
...
def fix_swapped_charge_discharge_columns(
    data: pl.DataFrame,
    tolerance: float = CAPACITY_ENERGY_INTEGRAL_TOLERANCE,
) -> pl.DataFrame:
    """Swap charge/discharge capacity and energy columns if their labels are inverted.

    Bails out (returning ``data`` unchanged) when positive current does
    not correspond to discharge — without a known sign convention there's
    no ground-truth direction to compare the cumulative columns against,
    and swapping the labels would mask a sign-convention bug. Run
    :func:`ionworksdata.transform.set_positive_current_for_discharge`
    first if you suspect this.

    Otherwise, for each pair (capacity, energy), compares the reported
    columns to the trapezoidal integral of ``max(I, 0)`` / ``max(-I, 0)``
    (and similarly for power, which is taken from ``"Power [W]"`` or
    derived as ``"Voltage [V]" * "Current [A]"`` when absent). If the
    swapped assignment lands within ``tolerance`` *and* the as-is
    assignment does not, the two columns are renamed.

    This is intended to recover from cyclers that mislabel charge vs.
    discharge (common with half-cell exports).

    Parameters
    ----------
    data : pl.DataFrame
        Time series with ``"Time [s]"``, ``"Step count"``, ``"Current [A]"``,
        and at least one of the four cumulative columns. ``"Voltage [V]"``
        and/or ``"Power [W]"`` are required to evaluate the energy pair.
    tolerance : float, optional
        Maximum allowed worst-row relative error between the reported
        column and the trapezoidal integral, evaluated as
        ``max|reported - integrated| / max(reported, integrated)``.
        Defaults to ``0.10`` (10 %).

    Returns
    -------
    pl.DataFrame
        ``data`` with each mislabeled column pair swapped. Returns ``data``
        unchanged when no swap is warranted or when the sign convention is
        unknown.
    """
    required = {"Time [s]", "Step count", "Current [A]"}
    if not required.issubset(data.columns):
        return data

    # Sign-convention guard: confirm positive current = discharge before
    # touching the labels. Without a known direction the "Discharge" /
    # "Charge" labels have no meaning to compare against — swapping them
    # would mask a sign-convention bug.
    if validate_positive_current_is_discharge(data):
        warnings.warn(
            "fix_swapped_charge_discharge_columns: positive current does "
            "not correspond to discharge; refusing to swap labels. Run "
            "set_positive_current_for_discharge first.",
            stacklevel=2,
        )
        return data

    time = data.get_column("Time [s]").to_numpy().astype(float, copy=False)
    if len(time) < 2:
        return data
    current = data.get_column("Current [A]").to_numpy().astype(float, copy=False)
    boundary = step_boundary_idx(data.get_column("Step count").to_numpy())

    def integrate(signed: np.ndarray) -> np.ndarray:
        return running_step_reset_integral(signed, time, boundary_idx=boundary)

    def rel_err(reported: np.ndarray, integrated: np.ndarray) -> float:
        return worst_row_relative_error(reported, integrated)[1]

    pairs: list[tuple[str, str, np.ndarray, np.ndarray]] = [
        (
            "Discharge capacity [A.h]",
            "Charge capacity [A.h]",
            integrate(np.maximum(current, 0.0)),
            integrate(np.maximum(-current, 0.0)),
        ),
    ]
    if "Power [W]" in data.columns:
        power = data.get_column("Power [W]").to_numpy().astype(float, copy=False)
    elif "Voltage [V]" in data.columns:
        voltage = data.get_column("Voltage [V]").to_numpy().astype(float, copy=False)
    ...
    if power is not None:
        pairs.append(
            (
                "Discharge energy [W.h]",
                "Charge energy [W.h]",
                integrate(np.maximum(power, 0.0)),
                integrate(np.maximum(-power, 0.0)),
            )
        )

    swaps: list[tuple[str, str]] = []
    for discharge_col, charge_col, discharge_integral, charge_integral in pairs:
        if discharge_col not in data.columns or charge_col not in data.columns:
            continue
        reported_discharge = (
            data.get_column(discharge_col).to_numpy().astype(float, copy=False)
        )
        reported_charge = (
            data.get_column(charge_col).to_numpy().astype(float, copy=False)
        )
        as_is = max(
            rel_err(reported_discharge, discharge_integral),
            rel_err(reported_charge, charge_integral),
        )
        swapped = max(
            rel_err(reported_discharge, charge_integral),
            rel_err(reported_charge, discharge_integral),
        )
        if swapped <= tolerance < as_is:
            swaps.append((discharge_col, charge_col))
