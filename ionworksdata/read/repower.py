...
from ._metadata import localize
...
class Repower(BaseReader):
    ...
    def read_metadata(
        ...
        """
        Report the reader's name and start time: the format carries no more.

        Overrides the base reader only to record why nothing is added. The
        format has no header block, schedule name, channel or instrument
        field.

        Parameters
        ----------
        filename : str | Path
            Path to the Repower CSV file.
        options : dict[str, str] | None, optional
            Options for reading the file. See
            :func:`ionworksdata.read.Repower.run`.

        Returns
        -------
        dict[str, Any]
            ``test_setup["cycler"]`` plus ``start_time``, from the ``System
            Time`` column's earliest value.

            ``Warm Temp setting`` and ``humidity setting`` are not recorded:
            both read ``0`` on a rig with no chamber, indistinguishable from a
            real 0 °C setpoint.

        Notes
        -----
        ``cell_metadata`` is defaulted when omitted. Only this reader marks it
        ``[required]``, for the cut-offs :meth:`run` labels steps with; reading
        one column needs nothing about the cell.
        """
        options = {"cell_metadata": {}, **(options or {})}
        return super().read_metadata(filename, options)
