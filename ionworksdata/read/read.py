...
from ionworksdata.read._metadata import take_known_start_time
...
class BaseReader:
    ...
    ALWAYS_NUMERIC_COLUMNS = [
        ...
        "Power [W]",
        "Charge capacity [A.h]",
        "Charge capacity [mA.h]",
        "Discharge capacity [A.h]",
        "Discharge capacity [mA.h]",
        "Charge energy [W.h]",
        "Charge energy [mW.h]",
        "Discharge energy [W.h]",
        "Discharge energy [mW.h]",
    ]
    ...
    def standard_data_processing(
        ...
        skip_cols = set(self.ALWAYS_NUMERIC_COLUMNS) | {
            "Step from cycler",
            "Cycle from cycler",
        }
        for col, dtype in data.schema.items():
            ...
            if dtype in NUMERIC_DTYPES:
                ...
                # Try to coerce string columns that contain numeric data
                # (e.g. extra sensor columns read as strings by the Maccor reader)
                coerced = (
                    data[col].str.replace_all(",", "").cast(pl.Float64, strict=False)
                )
                if coerced.null_count() <= data[col].null_count():
                    data = data.with_columns(coerced.alias(col))
    ...
    def read_metadata(
        ...
        """
        Read descriptive metadata that the file header carries, if any.

        Grouped as the measurement API defines its fields, so the result needs
        no reshaping. This default reports the reader's name plus whatever
        :meth:`read_start_time` yields, so a caller can ask any reader without
        branching on type.

        Returns
        -------
        dict[str, Any]
            ``test_setup["cycler"]`` -- the reader's name, which identifies the
            *format*, not the lab's equipment, so a caller who knows the model
            (``"Novonix UHPC"``) sets it themselves and merging keeps theirs.
            Plus ``start_time`` when the reader can determine one. Keys the file
            does not carry are omitted.

        See Also
        --------
        ionworksdata.read._metadata.CANONICAL_METADATA_KEYS : the shared
            vocabulary the overriding readers draw their keys from, so
            ``test_setup`` keeps one shape across cyclers. Maccor, Novonix,
            Basytec, Biologic, Arbin, Neware, Gamry and Digatron override this;
            BDF, parquet, plain CSV, BioLogic CSV and Repower have no header
            and keep this default.
        """
        known, options = take_known_start_time(options)
        metadata: dict[str, Any] = {"test_setup": {"cycler": self.name}}
        try:
            started = known or self.read_start_time(filename, options=options)
        except NotImplementedError:
            # Unreachable for the readers in this package -- all 16 inherit an
            # implementation. Kept for a third-party subclass that does not.
            return metadata
        if started is not None:
            metadata["start_time"] = started
        return metadata
...
def time_series(
    ...
    pl.DataFrame
        ...
        - "Power [W]" : Power in watts (Voltage [V] * Current [A])
    ...
    data = iwdata.transform.derive_impedance_components(data)
    ...
    data = iwdata.transform.clear_time_domain_columns_on_eis_rows(data)
...
def time_series_and_steps(
    ...
    if should_validate:
        try:
            ...
            if e.has_code(IssueCode.CURRENT_SIGN_REVERSED) or e.has_code(
                ...
                # If a native (cycler-supplied) "Power [W]" column is present, its
                # magnitude is authoritative but its sign was derived under the old,
                # now-corrected current convention. Re-sign it from the corrected
                # current instead of discarding it, the same way
                # _split_cumulative_by_direction preserves a native capacity/energy
                # column's magnitude while re-deriving direction from current.
                data = iwdata.transform.resign_power_from_current(data)
                # Drop the stale capacity/energy columns from the first pass before
                # recomputing. _calculate_capacity / _calculate_energy short-circuit
                # when these columns already exist (just taking abs()), which would
                # leave the charge/discharge labels swapped under the new sign
                # convention. Force the integration path by clearing them first.
                stale = [
                    c
                    for c in (
                        "Discharge capacity [A.h]",
                ...
                # Recalculate capacity and energy with correct sign. If "Power [W]"
                # was native, it is already re-signed above, so _calculate_energy's
                # direction-split (transform.py, which prefers an existing Power
                # column over Voltage*Current) uses the corrected sign too.
                ...
                # No-op if "Power [W]" already exists (native and re-signed above);
...
def keep_required_columns(
    ...
    pl.DataFrame
        ...
        - "Step count"
        - "Cycle count"
        - "Frame" (only readers that recover calls write it)
        - "Discharge capacity [A.h]"
        - "Charge capacity [A.h]"
        - "Discharge energy [W.h]"
        - "Charge energy [W.h]"
...
def metadata(
    ...
    """
    Read descriptive metadata from a cycler file's header.
    ...
    options : dict[str, Any] | None, optional
        A dictionary of options to pass to the reader. See the reader's
        documentation for the available options. Default is None.

    Returns
    -------
    dict[str, Any]
        The metadata the header carries, grouped the way the measurement API
        defines it -- ``protocol`` and ``test_setup`` sub-dicts, with
        ``start_time`` top-level -- so it can be merged into a measurement dict
        as-is. Every reader draws its keys from one shared vocabulary
        (:data:`ionworksdata.read._metadata.CANONICAL_METADATA_KEYS`), so the
        shape does not vary by cycler; which of those keys a given format can
        fill does. For a format with no header this is the reader's name and,
        where it can be determined, the start time. See the specific reader's
        ``read_metadata`` for the keys it can return, for example
        :meth:`ionworksdata.read.Maccor.read_metadata`.
    """
    _validate_argument_order(filename, reader, "metadata")
    ...
    return BaseReader.get_reader_object(reader).read_metadata(filename, options)
...
def _merge_header_metadata(measurement: dict[str, Any], header: dict[str, Any]) -> None:
    """
    Merge reader metadata into a measurement dict without overwriting it.

    Parameters
    ----------
    measurement : dict[str, Any]
        Measurement dict, updated in place.
    header : dict[str, Any]
        Metadata from :func:`metadata`, already grouped as the API defines it
        (see :meth:`ionworksdata.read.Maccor.read_metadata`).

    Notes
    -----
    Values the caller already set win, one level into a group as well as at the
    top: a caller's own ``protocol["name"]`` is kept while the reader still
    contributes the other ``protocol`` keys.
    """
    for key, value in header.items():
        if isinstance(value, dict):
            group = measurement.setdefault(key, {})
            for inner_key, inner_value in value.items():
                group.setdefault(inner_key, inner_value)
        else:
...
def measurement_details(
    ...
    """
    Read a cycler file into the time series, steps and measurement dict for upload.

    The time series comes from :func:`ionworksdata.read.time_series_and_steps`,
    reduced to the required columns by
    :func:`ionworksdata.read.keep_required_columns`. The cycler name, test start
    time and any header metadata are added to the measurement dictionary.
    ...
    dict[str, Any]
        ...
        - "measurement": dict with the updated measurement information, with
          header metadata nested as the API defines it -- see
          :func:`_merge_header_metadata`.
    """
    if data_type == "ocp":
    ...
    header = metadata(filename, reader, options)
    started = header.pop("start_time", None)
    if started is not None:
        measurement["start_time"] = started.isoformat()
    # The reader reports its display name; this path has the caller's key,
    # which is what it has always stored, so that wins here.
    header.setdefault("test_setup", {})["cycler"] = reader
    _merge_header_metadata(measurement, header)
