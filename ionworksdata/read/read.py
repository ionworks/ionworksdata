...
class BaseReader:
    ...
    def read_metadata(
        ...
        Grouped the way the measurement API defines its fields, so the result
        can be used to create a measurement without reshaping. The default
        reports the reader's own name plus whatever :meth:`read_start_time`
        yields, so a caller can ask any reader without branching on type.
        Readers with richer headers override this and add their own keys -- see
        ...
        try:
            started = self.read_start_time(filename, options=options)
...
def metadata(
    ...
    dict[str, Any]
        ...
        as-is. Empty for readers whose formats expose no header metadata. See
        the specific reader's ``read_metadata`` for the keys it can return, for
        example :meth:`ionworksdata.read.Maccor.read_metadata`.
    """
