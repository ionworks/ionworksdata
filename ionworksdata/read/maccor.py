...
from ._metadata import (
    channel_from_extension as _channel_from_extension,
    localize as _localize,
    nest_metadata as _nest_metadata,
...
#: The per-sample procedure activation, derived from the mode codes.
FRAME_COLUMN = "Frame"


def _frame_numbers(mode: np.ndarray) -> np.ndarray:
    """Number each sample by the procedure activation that wrote it.

    Maccor writes a zero-length step at every control transfer. A call is the
    pair ``P``, ``S`` and a return is ``O``; the markers themselves belong to
    the activation that wrote them, so the call to the caller and the return
    to the callee. An ``O`` with nothing to return to ends the master
    procedure. ``P`` or ``S`` on their own, or ``P`` twice, are other control
    steps that transfer nothing.

    Parameters
    ----------
    mode : numpy.ndarray
        The single-letter mode codes, one per sample.

    Returns
    -------
    numpy.ndarray
        One ``int64`` per sample. The master procedure is 0 and each later
        activation takes the next number in the order it began, so a
        sub-procedure called twice is two activations.
    """
    frame = np.zeros(len(mode), dtype=np.int64)
    markers = np.flatnonzero(np.isin(mode, ["P", "S", "O"]))
    stack = [0]
    started = 0
    # Frame of each sample from ``changed_at[k]`` up to the next change.
    changed_at, value = [0], [0]
    k = 0
    while k < len(markers):
        i = int(markers[k])
        if mode[i] == "P" and k + 1 < len(markers) and markers[k + 1] == i + 1:
            if mode[i + 1] == "S":
                started += 1
                stack.append(started)
                changed_at.append(i + 2)
                value.append(started)
                k += 2
                continue
        if mode[i] == "O" and len(stack) > 1:
            stack.pop()
            changed_at.append(i + 1)
            value.append(stack[-1])
        k += 1
    stops = [*changed_at[1:], len(mode)]
    for start, stop, frame_value in zip(changed_at, stops, value, strict=True):
        frame[start:stop] = frame_value
    return frame
...
class Maccor(BaseReader):
    ...
    def run(
        ...
        # After every row filter, since the numbering reads the samples in order.
        data = self.set_frame(data)

        return data

    @staticmethod
    def set_frame(data: pl.DataFrame) -> pl.DataFrame:
        """Add the procedure activation each sample belongs to, from its mode.

        Called by :meth:`run`, and usable on a time series read before the
        reader wrote the column, provided it kept ``Mode from cycler``.

        Parameters
        ----------
        data : pl.DataFrame
            A Maccor time series, in sample order.

        Returns
        -------
        pl.DataFrame
            The same frame with a ``Frame`` column. Returned unchanged when it
            has no ``Mode from cycler``: the calls cannot be recovered, and no
            column says so more plainly than one that is absent.
        """
        if _MACCOR_MODE_COLUMN not in data.columns:
            return data
        mode = data[_MACCOR_MODE_COLUMN].cast(pl.Utf8).fill_null("").to_numpy()
        return data.with_columns(
            pl.Series(FRAME_COLUMN, _frame_numbers(mode), dtype=pl.Int64)
        )
