...
class Maccor(BaseReader):
    ...
    def run(
        ...
        Read and process data from a Maccor file. The following column mappings are applied by default:
            ...
            - "Test Time (Hr)" -> "Time [h]"
            - "Cycle", "Cyc#", "Cycle ID" -> "Cycle from cycler"
            - "Cycle C" (preferred) or "Cycle P" (fallback) -> "Cycle from cycler"
              ("Cycle C" is the cumulative cycle counter and is always the true
              cycle number; "Cycle P" is a procedure-level loop counter whose
              meaning depends on how the Maccor procedure was written — often 0,
              sometimes equal to "Cycle C". "Cycle C" wins when both are present)
            - "Step", "Step ID" -> "Step from cycler"
            - "LogTemp001", "Temperature (°C)", "EVTemp (C)" -> "Temperature [degC]"
            - "Status", "State", "MD" -> "Status"
            - "Capacity (Ah)", "Capacity (AHr)", "Cap. (Ah)", "Amp-hr" -> "Capacity [A.h]"
            - "Energy (Wh)", "Energy (WHr)", "Watt-hr" -> "Energy [W.h]"
            ...
            - "DPT", "DPT Time", "DPt Time" -> "Timestamp" (parsed as datetime, used to compute "Time [s]")
            - "Test Time", "TestTime" -> "Time [s]" (if Excel ``:D:HH:MM:SS`` or compact
              ``Nd HH:MM:SS[.fff]`` duration), or "Timestamp" (if datetime string),
              or treated as numeric seconds otherwise
