...
class CSV(BaseReader):
    ...
    def run(
        ...
        # Determine which standard columns are already provided by extra_column_mappings
        mapped_targets = {v.replace(" ", "") for v in extra_column_mappings.values()}

        # Voltage
        if "Voltage[V]" not in mapped_targets:
        ...
        # Current — try absolute current first, fall back to areal current.
        # Each check is independent so mapping either one skips only that detection.
        current_found = "Current[A]" in mapped_targets
        if not current_found:
            try:
                ...
                data["Current [A]"] = (
                    data[current_column] * current_scale + current_shift
                )
                current_found = True
            except ValueError:
                pass

        if not current_found and "Current[mA.cm-2]" not in mapped_targets:
            (
        ...
        # Time
        if "Time[s]" not in mapped_targets:
            time_column, time_scale, time_shift = _find_column(
                data.columns, TIME_COLUMNS
            )
            data["Time [s]"] = data[time_column] * time_scale + time_shift

        # Temperature
        if "Temperature[degC]" not in mapped_targets:
            try:
