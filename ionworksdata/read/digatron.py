...
from ._metadata import nest_metadata, take_known_start_time
...
#: The schedule is a per-row column here, not a header field. ``Procedure
#: Level`` alongside it is a nesting depth, not part of the name.
_DIGATRON_PROCEDURE_COLUMN = "procedure"
...
class Digatron(BaseReader):
    ...
    def read_metadata(
        ...
        """
        Read the descriptive metadata a Digatron export carries.

        A Digatron CSV has no header block at all -- column names on line one,
        data below -- so the one descriptive field it carries, the schedule
        name, is a per-row ``Procedure`` column read out of the data.

        Parameters
        ----------
        filename : str | Path
            Path to the Digatron CSV file.
        options : dict[str, str] | None, optional
            Options for reading the file. See
            :func:`ionworksdata.read.Digatron.run`.
        ...
        dict[str, Any]
            ...
            - ``protocol["name"]``: the ``Procedure`` column's value, e.g.
              ``"DigiBatt_WP3_HPPC"`` -- but **only when it is the same on
              every row**. A file whose rows name several procedures is a
              concatenation of runs, and no single value names its schedule, so
              the key is omitted rather than reporting whichever came first.
            - ``test_setup["cycler"]``: this reader's name.
            - ``start_time``: timezone-aware, from the ``Timestamp`` column,
              which unlike the other formats' carries its own UTC offset.
        """
        ...
        parsed: dict[str, Any] = {}
        procedure = self._scan_procedure(filename, opts["file_encoding"])
    ...
    @classmethod
    def _scan_procedure(cls, filename: str | Path, file_encoding: str) -> str | None:
        """
        Read the ``Procedure`` column's value, when the file states just one.
        ...
        -------
        str | None
            The single procedure name, or None when the column is absent, is
            empty, or holds more than one value.

        Notes
        -----
        Scanning projects to the one column rather than parsing all 22: these
        exports run to tens of MB, and a caller asking for metadata has usually
        asked for the time series too. Falls back to a full read for a
        non-UTF-8 file, since ``scan_csv`` takes no encoding.
        """
        try:
            if file_encoding.lower().replace("-", "") in {"utf8", "utf8lossy"}:
                lazy = pl.scan_csv(filename)
                names = lazy.collect_schema().names()
            else:
                lazy = pl.read_csv(filename, encoding=file_encoding).lazy()
                names = lazy.collect_schema().names()
            column = next(
                (c for c in names if c.strip().lower() == _DIGATRON_PROCEDURE_COLUMN),
                None,
            )
            if column is None:
                return None
            values = (
                lazy.select(pl.col(column).unique())
                .collect()
                .get_column(column)
                .drop_nulls()
                .to_list()
            ...
            # Metadata is descriptive: a file this cannot scan is one the
            # time-series read will refuse with a better message.
            return None
        if len(values) != 1:
            return None
