...
#: The metadata sheet an Arbin MITS workbook leads with.
_ARBIN_GLOBAL_INFO_SHEET = "global_info"

#: ``MASS`` / ``Specific Capacity`` / ``Capacity`` are omitted as cell
#: properties, and the ``Has *`` / ``Log * Flag`` columns as export switches.
_ARBIN_GLOBAL_INFO_LABELS: dict[str, str] = {
    "channel": "channel_number",
    "schedule file name": "procedure",
    "creator": "operator",
    "comments": "notes",
...
#: The same fields in a ``.res`` file's ``Global_Table``, which names them with
#: underscores and adds the two the sheet keeps in its top block.
_ARBIN_RES_LABELS: dict[str, str] = {
    "Channel_Number": "channel_number",
    "Schedule_File_Name": "procedure",
    "Creator": "operator",
    "Comments": "notes",
    "Software_Version": "software_version",
    "Serial_Number": "cycler_serial_number",
    "Test_Name": "test_name",
}

#: Banner-block labels. Their values sit right of the label in one export and
#: below it in another, so both places are checked.
_ARBIN_GLOBAL_INFO_BANNER_LABELS: dict[str, str] = {
    "test name": "test_name",
    "serial number": "cycler_serial_number",
}

#: The only two cells no export labels, so position is all there is. Counted
#: from the banner row, so a sheet without one contributes neither.
_ARBIN_GLOBAL_INFO_CELLS: dict[str, tuple[int, int]] = {
    "test_name": (1, 3),
    "source_file_path": (1, 6),
}

#: Row 0's banner, which the unlabelled cells above are counted from.
_ARBIN_GLOBAL_INFO_BANNER = "test report"

#: Row of ``Global_Info`` holding the column labels, with values on the next.
_ARBIN_GLOBAL_INFO_LABEL_ROW = 3

#: ``Start DateTime`` spellings across export versions.
_ARBIN_GLOBAL_INFO_START_FORMATS = (
    "%Y-%m-%d %H:%M:%S",
    "%Y-%m-%d %H:%M:%S.%f",
    "%m/%d/%Y %H:%M:%S",
    "%m/%d/%Y %H:%M:%S.%f",
...
class Arbin(BaseReader):
    ...
    @staticmethod
    def _find_global_info_sheet(filename: str | Path) -> str | None:
        """Return the workbook's ``Global_Info`` sheet name, if it has one."""
        return next(
            (
                n
                for n in list_sheet_names(filename)
                if n.strip().lower() == _ARBIN_GLOBAL_INFO_SHEET
            ),
            None,
        )

    @classmethod
    def _read_global_info(cls, filename: str | Path) -> dict[str, str]:
        """
        Read the metadata an Arbin workbook keeps on its ``Global_Info`` sheet.
        ...
        -------
        dict[str, str]
            Canonical key to value, empty when the workbook has no
            ``Global_Info`` sheet (a single-sheet export does not).

        Notes
        -----
        Banner fields are found by label: their position moves between
        exports, so a fixed ``(row, column)`` reads an empty cell for one of
        the two layouts. The test name and result path carry no label in any
        export and stay positional, counted from the ``TEST REPORT`` banner.
        """
        sheet = cls._find_global_info_sheet(filename)
        if sheet is None:
            return {}
        try:
            with suppress_excel_dtype_warnings():
        ...
        found: dict[str, str] = {}
        label_row = _ARBIN_GLOBAL_INFO_LABEL_ROW
        for row_index in range(min(label_row, grid.height)):
            for column, raw in enumerate(grid.row(row_index)):
                if raw is None:
                    continue
                key = _ARBIN_GLOBAL_INFO_BANNER_LABELS.get(str(raw).strip().lower())
                if key is None:
                    continue
                # Right of the label, else below it -- exports differ, and an
                # empty cell in one place means the value is in the other.
                value = cell(row_index, column + 1) or cell(row_index + 1, column)
                if value is not None:
                    found.setdefault(key, value)

        if cell(0, 3) and str(cell(0, 3)).strip().lower() == _ARBIN_GLOBAL_INFO_BANNER:
            for key, (row, column) in _ARBIN_GLOBAL_INFO_CELLS.items():
                value = cell(row, column)
                if value is not None:
                    found.setdefault(key, value)

        label_row = _ARBIN_GLOBAL_INFO_LABEL_ROW
        if label_row + 1 < grid.height:
            labels = grid.row(label_row)
            for column, label in enumerate(labels):
                if label is None:
                    continue
                key = _ARBIN_GLOBAL_INFO_LABELS.get(str(label).strip().lower())
                if key is None:
                    continue
                value = cell(label_row + 1, column)
                if value is not None:
                    found[key] = value
        return found

    @staticmethod
    def _read_res_global_table(filename: str | Path) -> dict[str, str]:
        """
        Read the metadata a ``.res`` file keeps in its ``Global_Table``.
        ...
        -------
        dict[str, str]
            Canonical key to value, from the table's first row -- one row per
            test, and a ``.res`` file holds one test.
        """
        csv_text = _mdb_export(filename, "Global_Table")
        ...
        if row is None:
            return {}
        found: dict[str, str] = {}
        for column, key in _ARBIN_RES_LABELS.items():
    ...
    def read_metadata(
        ...
        """
        Read the descriptive metadata an Arbin export carries.

        Parameters
        ----------
        filename : str | Path
            Path to the Arbin file (``.res``, ``.xls``/``.xlsx``, or ``.csv``).
        options : dict[str, str] | None, optional
            Options containing the timezone string (default ``"UTC"``), applied
            to ``Start DateTime``, which carries no offset.
        ...
        dict[str, Any]
            ...
            - ``protocol["name"]``: the ``Schedule File Name`` (``.sdu``), the
              schedule that produced the file.
            - ``test_setup["channel_number"]``: ``Channel`` / ``Channel_Number``.
            - ``test_setup["test_name"]``: the test's name, from the sheet's
              ``TEST REPORT`` block or the table's ``Test_Name``.
            - ``test_setup["source_file_path"]``: the result path the export
              came from (workbook only; the ``.res`` table has no such column).
            - ``test_setup["cycler_serial_number"]``: ``Serial Number``.
            - ``test_setup["software_version"]``: ``Software Version``, e.g.
              ``"PV7.0 Build: 181116USB"``.
            - ``test_setup["operator"]``: ``Creator``.
            - ``test_setup["notes"]``: ``Comments``.
            - ``test_setup["cycler"]``: this reader's name.
            - ``start_time``: timezone-aware. From ``Start DateTime`` where the
              header states one, else from :meth:`read_start_time`, which for
              a ``.csv`` (whose only record of the time is its first row) is
              the sole source.

            Keys the file does not carry are omitted, and Arbin commonly
            leaves ``Creator`` and ``Comments`` empty, which counts as absent.

            A ``.csv`` export and a single-sheet workbook carry no metadata
            block at all, so for those the result is the reader's name plus
            ``start_time`` -- the same as the base reader would give.

            ``MASS``, ``Specific Capacity`` and ``Capacity`` are deliberately
            not returned: they describe the cell, so they belong to the cell
            record rather than to one measurement's test setup.
        """
        ...
        ext = Path(filename).suffix.lower()
        if ext == ".res":
            parsed: dict[str, Any] = dict(self._read_res_global_table(filename))
        elif ext in (".xls", ".xlsx"):
            parsed = dict(self._read_global_info(filename))
        else:
            parsed = {}

        started = parsed.pop("start_time", None)
        naive = (
            parse_datetime(started, _ARBIN_GLOBAL_INFO_START_FORMATS)
        ...
        if naive is not None:
            ...
            # No header time, or one written in a spelling we do not know:
            # fall back to the first data row, which every dialect has.
