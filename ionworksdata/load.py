...
class DataLoader:
    ...
    def from_db(
        ...
        time_range : dict | None, optional
            ...
            ``Time [s]`` values written into ``to_config()``. Server-side job
            resolve truncates the live series to that window so a re-run is
            unaffected by later appends; local ``from_db`` still loads the full
            current series. This is a property of the pipeline config, not the
