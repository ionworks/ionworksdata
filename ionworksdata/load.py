...
def _as_option_mapping(options):
    """Accept either an options mapping or a schema options object.

    The two are interchangeable everywhere else on this surface, so a caller
    holding a ``DataLoaderOptions`` should not have to unwrap it by hand before
    merging it with keyword options.

    Parameters
    ----------
    options : Mapping or DataLoaderOptions or None
        Options as given by the caller.

    Returns
    -------
    dict
        A plain mapping, empty when nothing was supplied.
    """
    if options is None:
        return {}
    if hasattr(options, "to_config"):
        return options.to_config()
    return dict(options)


#: Step types with a controlled-variable endpoint in voltage. Rests, EIS and
#: unclassified steps are absent: none of them has one.
_VOLTAGE_EVENT_STEPS = frozenset(
    {
...
#: Endpoint resolution. Repeated steps share a termination at mV/0.1 mA, so
#: PyBaMM builds one model for the lot rather than one per step.
_VOLTAGE_EVENT_DP = 3
_CURRENT_EVENT_DP = 4


def _voltage_event(step_row: dict) -> str | None:
    """Where a step's voltage ended, as a PyBaMM termination string.
    ...
    -------
    str or None
        The endpoint, e.g. ``"3.0 V"``, or None for a step with no
        controlled-variable endpoint in voltage.
    """
    if step_row["Step type"] not in _VOLTAGE_EVENT_STEPS:
        return None
    end = step_row.get("End voltage [V]")
    if end is None:
        return None
    return f"{round(end, _VOLTAGE_EVENT_DP)} V"


#: Slack on a window bound, absorbing the float round-trip of a stored pin.
_TIME_WINDOW_TOLERANCE_S = 1e-6


def _validated_time_range(time_range):
    """Return *time_range* if it is a usable window, else raise.

    Parameters
    ----------
    time_range : dict or None
        ``{"start": seconds, "end": seconds}``, either bound optional.
    ...
    ------
    ValueError
        If it is not a mapping, or a bound is not a finite number.
    """
    if time_range is None:
        return None
    if not isinstance(time_range, Mapping):
        raise ValueError(
            f"time_range must be a mapping of start/end seconds, got "
            f"{type(time_range).__name__}"
        )
    for key in ("start", "end"):
        bound = time_range.get(key)
        if bound is None:
            continue
        if isinstance(bound, bool) or not isinstance(bound, (int, float)):
            raise ValueError(
                f"time_range.{key} must be a number of seconds, got {bound!r}"
            )
        if not math.isfinite(float(bound)):
            raise ValueError(f"time_range.{key} must be finite, got {bound!r}")
    start, end = time_range.get("start"), time_range.get("end")
    if start is not None and end is not None and float(end) < float(start):
        raise ValueError(f"time_range starts at {start} s and ends at {end} s")
    return dict(time_range)


def window_rows(frame, time_range):
    """Keep the rows of *frame* whose ``Time [s]`` falls inside *time_range*.

    For data carrying no steps, where there is no slice to narrow and the whole
    frame is what a fit sees.
    ...
    time_range : dict or None
        The window; None returns the frame unchanged.

    Returns
    -------
    pd.DataFrame
    """
    window = _validated_time_range(time_range)
    if not window or "Time [s]" not in getattr(frame, "columns", ()):
        return frame
    lo, hi = window.get("start"), window.get("end")
    times = frame["Time [s]"]
    keep = None
    if lo is not None:
        keep = times >= lo - _TIME_WINDOW_TOLERANCE_S
    if hi is not None:
        upper = times <= hi + _TIME_WINDOW_TOLERANCE_S
        keep = upper if keep is None else keep & upper
    if keep is None:
        return frame
    kept = frame.loc[keep]
    if kept.empty and not frame.empty:
        # Said rather than returned: a fit handed nothing fails somewhere far
        # from the window that emptied it. The stepped path says the same.
        raise ValueError(
            f"time_range {lo}-{hi} s selects no samples of this measurement, "
            f"which covers {float(times.min())}-{float(times.max())} s."
        )
    return kept
...
class DataLoader:
    ...
    ``**kwargs``
        Options passed directly or via an ``options`` dict. The set below is
        closed — an unrecognised key raises rather than being ignored, so a
        misspelt option cannot silently skip the preprocessing you asked for.
        Supported keys:

        When steps are provided:

            - first_step, last_step : int | str | dict
                A step index, a SQL query selecting one row of the steps table,
                or the deprecated cycle/step dict.
            - first_step_dict, last_step_dict : deprecated spellings of the above
    ...
    def __init__(
        ...
        **kwargs,
        ...
        options = {**_as_option_mapping(kwargs.pop("options", None)), **kwargs}
        self._measurement_id = None
        # Not an option — it sits beside `options` in a data config, which
        # `DataLoaderOptions` would reject — so it is checked here instead.
        self._time_range = _validated_time_range(options.pop("time_range", None))
    ...
    def _ensure_time_series_loaded(self):
        ...
        if self._steps_pl is None:
            ...
            start_idx, end_idx = self._apply_time_range(
                time_series_pl, self._start_idx, self._end_idx
            )
            self._steps_pl = self._clip_steps_to_window(
                self._steps_pl, time_series_pl, start_idx, end_idx
            )
            self._start_idx = start_idx
            self._end_idx = end_idx
    ...
    def _parse_options(options):
        ...
        # Imported lazily (like pybamm elsewhere in this module, via
        # _import_pybamm) so that importing ionworksdata alone stays light;
        # ionworks_schema pulls in pybamm at its own import time.
        from ionworks_schema import DataLoaderOptions

        # The schema owns this option surface; validating here rejects an unknown
        # key instead of ignoring it and quietly changing the result. This is the
        # sole entry point shared by __init__ (via _setup) and from_db's lazy
        # (cache-miss) path, so both are covered by one validation call.
        options = DataLoaderOptions.model_validate(options).model_dump(
            exclude_none=True
        )
    ...
    @staticmethod
    def _clip_steps_to_window(steps_pl, time_series_pl, start_idx, end_idx):
        """Keep the retained steps addressing rows the window still holds.

        Every reader of a step's ``Start index`` / ``End index`` subtracts
        ``_start_idx``, so a step the window cuts into would otherwise hand back
        an offset before the slice begins — which polars reads as counting from
        the end, quietly returning the wrong rows.

        Parameters
        ----------
        steps_pl : pl.DataFrame
            The chosen steps, with absolute row offsets.
        time_series_pl : pl.DataFrame
            The whole series the offsets address, unsliced.
        start_idx : int
            First row the window keeps.
        end_idx : int
            One past the last row it keeps.

        Returns
        -------
        pl.DataFrame
            Steps overlapping the window, their offsets clipped into it and
            their endpoint-derived columns re-read at the new endpoints.
        """
        columns = ("Start index", "End index")
        if steps_pl.height == 0 or not all(c in steps_pl.columns for c in columns):
            return steps_pl
        overlapping = steps_pl.filter(
            (pl.col("End index") >= start_idx) & (pl.col("Start index") < end_idx)
        )
        clipped = overlapping.with_columns(
            pl.col("Start index").clip(lower_bound=start_idx),
            pl.col("End index").clip(upper_bound=end_idx - 1),
        )
        if all(clipped[column].equals(overlapping[column]) for column in columns):
            # No bound moved, so the summary still describes the rows it did.
            return clipped
        return DataLoader._reread_step_endpoints(clipped, time_series_pl)

    @staticmethod
    def _reread_step_endpoints(steps_pl, time_series_pl):
        """Re-read the columns a step summarised from its first and last row.

        A clipped step still claiming the whole step's times is refused by
        :meth:`_check_indices_still_name_the_step` and replayed for the wrong
        duration. Distributional columns stay as they were: they need the
        samples, not the endpoints.

        Parameters
        ----------
        steps_pl : pl.DataFrame
            Steps whose offsets already lie inside the window.
        time_series_pl : pl.DataFrame
            The whole series those offsets address.
        ...
        """
        endpoints = {
            "Start time [s]": ("Time [s]", "Start index"),
            "End time [s]": ("Time [s]", "End index"),
            "Start voltage [V]": ("Voltage [V]", "Start index"),
            "End voltage [V]": ("Voltage [V]", "End index"),
        }
        # Accumulated columns are reported per step as a difference between the
        # step's own endpoints, so a window narrows them the same way.
        spans = (
        ...
        for name, (source, offset) in endpoints.items():
            if name in steps_pl.columns and source in time_series_pl.columns:
                steps_pl = steps_pl.with_columns(
                    time_series_pl[source].gather(steps_pl[offset]).alias(name)
                )
        for name in spans:
            if name in steps_pl.columns and name in time_series_pl.columns:
                column = time_series_pl[name]
                steps_pl = steps_pl.with_columns(
                    (
                        column.gather(steps_pl["End index"])
                        - column.gather(steps_pl["Start index"])
                    ).alias(name)
                )
        timed = {"Duration [s]", "Start time [s]", "End time [s]"} <= set(
            steps_pl.columns
        )
        if timed and "Time [s]" in time_series_pl.columns:
            steps_pl = steps_pl.with_columns(
                (pl.col("End time [s]") - pl.col("Start time [s]")).alias(
                    "Duration [s]"
                )
            )
        return steps_pl

    def _apply_time_range(self, time_series_pl, start_idx, end_idx):
        """Narrow a step slice to the rows inside ``time_range``.

        The window is applied *after* the steps are chosen, so ``first_step``
        always counts steps of the measurement rather than of whatever survived
        a filter. Narrowing the bounds rather than dropping rows is what keeps
        the steps table's own ``Start index`` / ``End index`` meaningful: every
        reader of those subtracts ``_start_idx``.

        Parameters
        ----------
        time_series_pl : pl.DataFrame
            The whole series, unsliced.
        start_idx : int
            First row of the chosen steps.
        end_idx : int
            One past their last row.

        Returns
        -------
        tuple[int, int]
            The same bounds, narrowed to the window.

        Raises
        ------
        ValueError
            If the window and the chosen steps do not overlap, which otherwise
            surfaces as an empty fit far from its cause.
        """
        # getattr: the lazy `from_db` paths build the instance without __init__,
        # as the other lazy attributes here are read.
        window = getattr(self, "_time_range", None)
        if not window or "Time [s]" not in time_series_pl.columns:
            return start_idx, end_idx
        lo, hi = window.get("start"), window.get("end")
        if lo is None and hi is None:
            return start_idx, end_idx

        # Bounds, not a row mask: a mask would renumber the rows the steps
        # table addresses. Sound because a series runs forwards in time.
        times = time_series_pl["Time [s]"].slice(start_idx, end_idx - start_idx)
        keep = pl.Series([True] * times.len())
        # Slack: a recorded window round-trips through float seconds, and
        # without it the boundary samples drop out.
        if lo is not None:
            keep = keep & (times >= lo - _TIME_WINDOW_TOLERANCE_S)
        if hi is not None:
            keep = keep & (times <= hi + _TIME_WINDOW_TOLERANCE_S)
        kept = keep.arg_true()
        if kept.len() == 0:
            raise ValueError(
                f"time_range {lo}-{hi} s selects no samples of the chosen "
                f"steps, which cover {float(times.min())}-{float(times.max())} s. "
                "Widen the window, or select steps inside it."
            )
        return start_idx + int(kept[0]), start_idx + int(kept[-1]) + 1
    ...
    def _init_with_steps(self, time_series, steps):
        ...
        start_idx, end_idx = self._apply_time_range(time_series_pl, start_idx, end_idx)
        sliced_steps_pl = self._clip_steps_to_window(
            sliced_steps_pl, time_series_pl, start_idx, end_idx
        )
        self._steps_pl = sliced_steps_pl
    ...
    def to_config(self, filter_data: bool = True) -> dict:
        ...
        if hasattr(self, "_measurement_id") and self._measurement_id is not None:
            ...
            if getattr(self, "_time_range", None) is not None:
                config["time_range"] = self._time_range
    ...
    def generate_experiment(
        ...
        use_cv : bool, optional
            Emit constant-voltage steps as voltage holds rather than as a
            current interpolant. Default False.
        termination : str, optional
            What ends each step. Default ``"duration"``.

            - ``"duration"``: every step ends after its measured
              ``Duration [s]``.
            - ``"events"``: every step ends where the measured step ended, in
              the variable it was controlling — a constant-current or
              constant-power step at its final voltage, a voltage hold at its
              final current — and carries no duration, so its length becomes
              the model's own. A capacity disagreement between model and cell
              then reads as capacity to that endpoint rather than as a
              residual voltage. Rests and steps emitted as a current
              interpolant keep their duration either way.

            ``"events"`` replays endpoints, not the cycler's intent: a step the
            cycler ended on a time limit is still replayed as ending at the
            voltage it reached, because the steps table cannot tell the two
            apart. Use it for endpoint-driven protocols (rate capability,
            CC-CV cycling) and ``"duration"`` for time-driven ones. Where a
            step's voltage barely moves, such as a short pulse, the endpoint
            can already be satisfied at the model's initial state; PyBaMM warns
            and skips such a step rather than solving it.
        period : str or float, optional
            Sampling period for the emitted steps, as
        ...
        -------
        pybamm.Experiment
            One PyBaMM cycle per run of equal ``"Cycle count"`` in the steps
        ...
        if termination not in ("duration", "events"):
            raise ValueError(
                f"termination must be 'duration' or 'events'; got {termination!r}"
            )
        ...
        for step_row in steps_pl.iter_rows(named=True):
            ...
            if duration is not None and duration <= np.nextafter(0, 1):
                continue
            step_period = (
                self._step_period(step_row) if period == "from data" else period
            )
            event = (
                self._step_event(step_row, use_cv) if termination == "events" else None
            )
            if event is not None:
                kwargs = {"termination": event, "period": step_period}
            elif duration is None:
                raise ValueError(
                    f"Step {step_row.get('Step count')} ({step_type}) has no "
                    "'Duration [s]' and did not end on a cut-off, so it cannot "
                    "be turned into a PyBaMM step."
                )
            else:
                kwargs = {"duration": duration, "period": step_period}
            match step_type:
                case "Constant current discharge" | "Constant current charge":
                    mean_current = step_row["Mean current [A]"]
                    step = pybamm.step.Current(mean_current, **kwargs)
                case "Constant voltage discharge" | "Constant voltage charge":
                    if use_cv:
                        mean_voltage = step_row["Mean voltage [V]"]
                        step = pybamm.step.Voltage(mean_voltage, **kwargs)
                    else:
                        step = self._create_current_interpolant_step(step_row, **kwargs)
                case "Rest":
                    step = pybamm.step.Current(0, **kwargs)
                case "Constant power discharge" | "Constant power charge":
                    mean_power = step_row["Mean power [W]"]
                    step = pybamm.step.Power(mean_power, **kwargs)
                case "EIS":
                    raise NotImplementedError(
                        "EIS steps are not yet implemented directly in PyBaMM "
                        "experiments. Please simulate EIS using pybamm.EISSimulation."
                    )
                case _:
                    unclassified.append(f"{step_row.get('Step count')} ({step_type})")
                    step = self._create_current_interpolant_step(step_row, **kwargs)
        ...
        if unclassified:
            ...
            # dedup cannot collapse them — hence one for the whole table.
            shown = ", ".join(unclassified[:5])
            if len(unclassified) > 5:
                shown += f", and {len(unclassified) - 5} more"
            warnings.warn(
                "No step type was recognised for these steps, so each is emitted "
                "as a pybamm.step.Current interpolating its measured current "
                "rather than as a control law, and takes no termination. A cycler "
                f"that merges phases into one step reads this way: {shown}.",
                stacklevel=2,
            )
    ...
    def _get_times_and_currents(self, step):
        # Use the data property so lazily-loaded (from_db) time series is fetched
        data = self.data
        start_idx = int(step["Start index"]) - self._start_idx
        end_idx = int(step["End index"]) + 1 - self._start_idx
        length = end_idx - start_idx
        current = data["Current [A]"].slice(start_idx, length).to_numpy()
        time = data["Time [s]"].slice(start_idx, length).to_numpy()
        self._check_indices_still_name_the_step(step, time)
        return time, current

    @staticmethod
    def _check_indices_still_name_the_step(step: dict, time: np.ndarray) -> None:
        """Reject a step whose indices no longer select its own samples.

        A transform that rebuilds rows — sorting, resampling, dropping
        duplicates — leaves the steps table's positional indices resolving onto
        other steps' samples rather than failing, so the step's own recorded
        span and times are what tell the two apart.
        ...
        step : dict
            ...
            :func:`ionworksdata.steps.summarize`.
        time : np.ndarray
            The times the indices selected.

        Raises
        ------
        ValueError
            If the selection is not as long as the step, or does not start and
            end when the step did.
        """
        span = int(step["End index"]) - int(step["Start index"]) + 1
        bounds = (step.get("Start time [s]"), step.get("End time [s]"))
        timed = len(time) and not any(bound is None for bound in bounds)
        # Absolute tolerance only: a relative one would admit a mismatch of
        # seconds late in a long run. Without times, length is all there is.
        if len(time) == span and (
            not timed or np.allclose((time[0], time[-1]), bounds, rtol=0, atol=1e-6)
        ):
            return
        selected = (
            f"{len(time)} samples from {time[0]} s to {time[-1]} s"
            if len(time)
            else "no samples at all"
        )
        raise ValueError(
            f"Step {step.get('Step count')} spans {span} samples and ran from "
            f"{bounds[0]} s to {bounds[1]} s, but its 'Start index' and 'End "
            f"index' select {selected}. The time series has been reordered or "
    ...
    def _step_event(self, step_row: dict, use_cv: bool) -> str | None:
        ...
        step_row : dict
            ...
            :func:`ionworksdata.steps.summarize`.
        use_cv : bool
            Whether constant-voltage steps are emitted as voltage holds.

        Returns
        -------
        str or None
            A PyBaMM termination string, or None for a step with no
            controlled-variable endpoint.
        """
        if step_row["Step type"].startswith("Constant voltage"):
            return self._hold_current_event(step_row) if use_cv else None
        return _voltage_event(step_row)

    def _hold_current_event(self, step_row: dict) -> str | None:
        """Where a voltage hold's current ended, as a termination string.

        Read from the series rather than from ``Min``/``Max current [A]``:
        those are untrimmed, so on a hold whose first sample still carries the
        previous step's setpoint, the smaller magnitude is that sample rather
        than the taper.
        ...
        -------
        str or None
            The endpoint, e.g. ``"0.05 A"``, or None for a hold that ended at
            no current: PyBaMM's event is ``|I| - value``, which then never
            crosses zero.
        """
        _, current = self._get_times_and_currents(step_row)
        if not len(current):
            return None
        reached = round(abs(float(current[-1])), _CURRENT_EVENT_DP)
        if not reached:
            return None
        return f"{reached} A"

    @staticmethod
    def _step_period(step_row: dict) -> float | None:
        """Mean sample interval over a step, as ``Duration [s]`` per sample.
        ...
        ------
        ValueError
            If the table gives no duration or no sample count for the step, so
            the interval the caller asked for cannot be derived.
        """
        duration = step_row.get("Duration [s]")
        start, end = step_row.get("Start index"), step_row.get("End index")
        intervals = None if start is None or end is None else int(end) - int(start)
        if duration is None or not intervals or intervals <= 0:
            raise ValueError(
                f"Step {step_row.get('Step count')} "
                f"({step_row.get('Step type')}) has no duration or sample count, "
                "so period='from data' cannot derive its sample interval."
            )
        return duration / intervals

    def _create_current_interpolant_step(self, step, **kwargs):
        pybamm = _import_pybamm()
        ...
        return pybamm.step.Current(arr, **kwargs)
    ...
    def from_db(
        ...
        time_range : dict | None, optional
            Optional ``{"start": seconds, "end": seconds}`` window of elapsed
            ``Time [s]`` values, applied after ``first_step`` / ``last_step``
            have chosen their steps and written into ``to_config()``. It pins a
            re-run to the data the first run saw, so later appends do not change
            the answer. A property of the pipeline config, not of the
            measurement's own start/end times.
        ...
        if use_cache:
            ...
            if (
                ...
                instance._time_range = time_range  # noqa: SLF001
                return instance
...
class OCPDataLoader(DataLoader):
    ...
    def __init__(self, data, **kwargs):
        ...
        options = _as_option_mapping(kwargs.pop("options", None))
