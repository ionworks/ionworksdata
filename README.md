...
- **`load`** — Load processed data for use in other Ionworks software (`DataLoader`, `OCPDataLoader`). `DataLoader.data` and `.steps` return Polars DataFrames; use `.data.to_pandas()` or `.steps.to_pandas()` for pandas.
